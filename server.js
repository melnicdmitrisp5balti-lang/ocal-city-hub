...
app.delete('/api/admin/invites/used', requireAdmin, async (req, res) => {
  try { await run("DELETE FROM invite_codes WHERE used=1"); await addHistory(req.user.username,'clear_used_invites',null,null); res.json({ok:true}); }
  catch(e) { res.status(500).json({error:e.message}); }
});
app.delete('/api/admin/invites/:id', requireAdmin, async (req, res) => {
  try { await run("DELETE FROM invite_codes WHERE id=?", [req.params.id]); res.json({ok:true}); }
  catch(e) { res.status(500).json({error:e.message}); }
});
app.delete('/api/admin/invites', requireAdmin, async (req, res) => {
  try { await run("DELETE FROM invite_codes"); await addHistory(req.user.username,'clear_all_invites',null,null); res.json({ok:true}); }
  catch(e) { res.status(500).json({error:e.message}); }
});
...
function parseAiJson(text) {
  try {
    ...
    if (p && (p.html !== undefined || p.css !== undefined || p.js !== undefined)) return JSON.stringify(p);
    return null;
  } catch(e) { return null; }
}
...
app.post('/api/ai', requireAuth, async (req, res) => {
  try {
    const CF_TOKEN = process.env.CF_API_TOKEN, CF_ACCOUNT = process.env.CF_ACCOUNT_ID;
    ...
    const sys = 'You are a web developer. Respond ONLY with JSON: {"html":"body content","css":"styles","js":"scripts"}. No markdown. Dark theme. Escape special chars in JSON strings.';
